<template>
  <div style="width: 100%; height: 100%">
    ...
    >
      ...
      <br />
      <span @click="show = !show" class="checkbox"><input class="play" type="checkbox" v-model="show" />扫描线显隐</span
      ><br />
      ...
      <span>扫描线颜色：</span><input class="colorbox" type="color" value="#ffff00" @input="colorchange" /><br />
    </div>
    <div
      class="box"
  ...
  methods: {
    handlePlay() {
      ...
      if (this.play) {
        ...
        // ];
        this._earth.camera.position = [1.901700101080121, 0.5971351061731751, 1125.452554631433]
        this._earth.camera.rotation = [2.4868995751603507e-14, -0.7856778432623708, 6.283185307179586]
    ...
    renderPoint() {
      this._earth.camera.position = [1.9017263361090628, 0.5970981493916543, 1099.1695380684096]
      this._earth.camera.rotation = [6.270009474790001, -0.6822022214619832, 6.283132222871597]
      var positions = []
      var entity
      var temp = true
      for (var i = 0; i < 100; i++) {
    ...
    playVideo() {
      ...
      const isCreateVideo = this._earth.sceneTree.root.children.some(item => {
        console.log(item, '===item')
        return item.czmObject.xbsjType === 'CustomPrimitive'
      })
      console.log(isCreateVideo, '===isCreateVideo===')
      if (!isCreateVideo) {
        let model = JSON.parse(JSON.stringify(this._earth.sceneTree.root.children))
        model = model.concat(videoModel.children)
        this._earth.sceneTree.root.children = model
        console.log(model, this._earth.sceneTree.root.children, '===this._earth.sceneTree.root.children===')
      }
